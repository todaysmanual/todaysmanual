  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      {loading ? <div className="admin-loading">Loading your publication…</div> : <>
        ...
        {view === "messages" && <section className="admin-view"><header><div><p className="admin-kicker">Inbox</p><h1>Contact messages</h1></div></header><div className="admin-message-list">{messages.map((item) => <article key={item.id} className={`admin-message admin-message--${item.status}`}><header><div><span className={`admin-pill admin-pill--${item.status}`}>{item.status}</span><strong>{item.subject}</strong></div><time dateTime={item.created_at}>{new Date(item.created_at).toLocaleString()}</time></header><div className="admin-message__sender"><strong>{item.name}</strong><a href={`mailto:${item.email}`}>{item.email}</a></div><p>{item.message}</p><footer><a className="admin-primary" href={`mailto:${item.email}?subject=${encodeURIComponent(`Re: ${item.subject}`)}`}>Reply by email</a>{item.status !== "read" && <button className="admin-secondary" onClick={() => void updateMessage(item, "read")}>Mark read</button>}{item.status !== "archived" && <button className="admin-secondary" onClick={() => void updateMessage(item, "archived")}>Archive</button>}<button className="admin-message__delete" onClick={() => void deleteMessage(item)}>Delete</button></footer></article>)}</div>{messages.length === 0 && <div className="admin-empty"><strong>No contact messages yet</strong><p>New submissions from the Contact page will appear here.</p></div>}</section>}
