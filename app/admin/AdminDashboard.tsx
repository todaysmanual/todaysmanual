...
import type { ArticleRecord, CategoryRecord, SiteConfig, SubscriberRecord } from "@/lib/cms/types";

type View = "overview" | "articles" | "categories" | "homepage" | "site" | "subscribers" | "media";
...
export function AdminDashboard() {
  ...
  const loadData = useCallback(async (currentUser: User) => {
    ...
    setAuthorized(true);
    const [articleResult, categoryResult, settingsResult, subscriberResult] = await Promise.all([
    ...
    await loadMedia();
    const firstError = articleResult.error ?? categoryResult.error ?? settingsResult.error ?? subscriberResult.error;
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      {loading ? <div className="admin-loading">Loading your publication…</div> : <>
        ...
        {view === "subscribers" && <section className="admin-view"><header><div><p className="admin-kicker">Audience</p><h1>Subscribers</h1></div><button className="admin-secondary" onClick={exportSubscribers}>Export CSV</button></header><div className="admin-table-wrap"><table><thead><tr><th>Email</th><th>Status</th><th>Source</th><th>Joined</th><th /></tr></thead><tbody>{subscribers.map((item) => <tr key={item.id}><td>{item.email}</td><td><span className={`admin-pill admin-pill--${item.status}`}>{item.status}</span></td><td>{item.source}</td><td>{new Date(item.created_at).toLocaleDateString()}</td><td><button onClick={() => void updateSubscriber(item, item.status === "active" ? "unsubscribed" : "active")}>{item.status === "active" ? "Unsubscribe" : "Reactivate"}</button></td></tr>)}</tbody></table>{subscribers.length === 0 && <div className="admin-empty"><strong>No subscribers yet</strong><p>New website signups will appear here.</p></div>}</div></section>}

        {view === "media" && <section className="admin-view"><header><div><p className="admin-kicker">Supabase Storage</p><h1>Media library</h1></div><label className="admin-primary admin-file-action">Upload images<input type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,image/avif" onChange={(event) => void Promise.all(Array.from(event.target.files ?? []).map(uploadMedia))} /></label></header><div className="admin-media-grid">{media.map((file) => { const url = client?.storage.from("site-media").getPublicUrl(`uploads/${file.name}`).data.publicUrl ?? ""; return <article key={file.id ?? file.name}><img src={url} alt="" /><div><strong title={file.name}>{file.name}</strong><span>{formatBytes(file.metadata?.size)}</span><button onClick={() => void navigator.clipboard.writeText(url).then(() => showNotice("success", "Image URL copied."))}>Copy URL</button><button className="danger" onClick={() => void deleteMedia(file)}>Delete</button></div></article>; })}</div>{media.length === 0 && <div className="admin-empty"><strong>No uploaded media yet</strong><p>Upload an image here or replace one while editing an article.</p></div>}</section>}
      </>}
    </main>
  </div>;
}
